...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        name: "VcCol", // Change in other components
        lId: null,
        lVersion: null,
        lDataUrl: null,
        lDataSize: null,
    ...
    computed: {
      vcCols(){
        return this.$store.getters.VcCols; // Change in other components
      }
    },
    methods:{
      openAdd(){
      ...
      add(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хоитите добавить ${this.name}?\n ${JSON.stringify(data)}`)){
            this.$store.dispatch('addVcCol', data); // Change in other components
            this.dialog = false;
          }
        }
      },
      ...
      change(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хотите обновить ${this.name} ${id}?\n${JSON.stringify(data)}`)){
            ...
            this.$store.dispatch('updateVcCol', {id, data}); // Change in other components
          }
        }
      },
      deleteData(data){
        if (data.current === true)
        {
          ...
          this.$store.dispatch('deleteVcCol', data); // Change in other components
        }
      }
    },
    mounted() {
      this.$store.dispatch('fetchVcCols'); // Change in other components
    }
  }
</script>
