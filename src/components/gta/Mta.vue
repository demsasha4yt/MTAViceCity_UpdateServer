...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        name: "Mta", // Change in other components
        lId: null,
        lVersion: null,
        lDataUrl: null,
        lDataSize: null,
    ...
    computed: {
      mtas(){
        return this.$store.getters.Mtas; // Change in other components
      }
    },
    methods:{
      openAdd(){
      ...
      add(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хоитите добавить ${this.name}?\n ${JSON.stringify(data)}`)){
            this.$store.dispatch('addMta', data); // Change in other components
            this.dialog = false;
          }
        }
      },
      ...
      change(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хотите обновить ${this.name} ${id}?\n${JSON.stringify(data)}`)){
            ...
            this.$store.dispatch('updateMta', {id, data}); // Change in other components
          }
        }
      },
      deleteData(data){
        if (data.current === true)
        {
          ...
          this.$store.dispatch('deleteMta', data); // Change in other components
        }
      }
    },
    mounted() {
      this.$store.dispatch('fetchMtas'); // Change in other components
    }
  }
</script>
