...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        name: "Models", // Change in other components
        lId: null,
        lVersion: null,
        lDataUrl: null,
        lDataSize: null,
    ...
    computed: {
      models(){
        return this.$store.getters.Models; // Change in other components
      }
    },
    methods:{
      openAdd(){
      ...
      add(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хоитите добавить ${this.name}?\n ${JSON.stringify(data)}`)){
            this.$store.dispatch('addModel', data); // Change in other components
            this.dialog = false;
          }
        }
      },
      ...
      change(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хотите обновить ${this.name} ${id}?\n${JSON.stringify(data)}`)){
            ...
            this.$store.dispatch('updateModel', {id, data}); // Change in other components
          }
        }
      },
      deleteData(data){
        if (data.current === true)
        {
          ...
          this.$store.dispatch('deleteModel', data); // Change in other components
        }
      }
    },
    mounted() {
      this.$store.dispatch('fetchModels'); // Change in other components
    }
  }
</script>
