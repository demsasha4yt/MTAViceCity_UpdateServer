...
<script>
    ...
    export default {
        ...
        data: function () {
          return {
            name: "Audio", // Change in other components
            lId: null,
            lVersion: null,
            lDataUrl: null,
            lDataSize: null,
        ...
        computed: {
          audios(){
            return this.$store.getters.Audios; // Change in other components
          }
        },
        methods:{
          openAdd(){
          ...
          add(){
            ...
            if (checks.gta(data)){
              if (confirm(`Вы действительно хоитите добавить ${this.name}?\n ${JSON.stringify(data)}`)){
                this.$store.dispatch('addAudio', data); // Change in other components
                this.dialog = false;
              }
            }
          },
          ...
          change(){
            ...
            if (checks.gta(data)){
              if (confirm(`Вы действительно хотите обновить ${this.name} ${id}?\n${JSON.stringify(data)}`)){
                ...
                this.$store.dispatch('updateAudio', {id, data}); // Change in other components
              }
            }
          },
          deleteData(data){
            if (data.current === true)
            {
             alert('Невозможно удалить CURRENT');
              ...
              this.$store.dispatch('deleteAudio', data); // Change in other components
            }
          }
        },
      mounted() {
          this.$store.dispatch('fetchAudios'); // Change in other components
      }
    }
</script>
