...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        name: "Text", // Change in other components
        lId: null,
        lVersion: null,
        lDataUrl: null,
        lDataSize: null,
    ...
    computed: {
      texts(){
        return this.$store.getters.Texts; // Change in other components
      }
    },
    methods:{
      openAdd(){
      ...
      add(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хоитите добавить ${this.name}?\n ${JSON.stringify(data)}`)){
            this.$store.dispatch('addText', data); // Change in other components
            this.dialog = false;
          }
        }
      },
      ...
      change(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хотите обновить ${this.name} ${id}?\n${JSON.stringify(data)}`)){
            ...
            this.$store.dispatch('updateText', {id, data}); // Change in other components
          }
        }
      },
      deleteData(data){
        if (data.current === true)
        {
          ...
          this.$store.dispatch('deleteText', data); // Change in other components
        }
      }
    },
    mounted() {
      this.$store.dispatch('fetchTexts'); // Change in other components
    }
  }
</script>
