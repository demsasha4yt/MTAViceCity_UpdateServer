...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        name: "Data", // Change in other components
        lId: null,
        lVersion: null,
        lDataUrl: null,
        lDataSize: null,
    ...
    computed: {
      datas(){
        return this.$store.getters.Datas; // Change in other components
      }
    },
    methods:{
      openAdd(){
      ...
      add(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хоитите добавить ${this.name}?\n ${JSON.stringify(data)}`)){
            this.$store.dispatch('addData', data); // Change in other components
            this.dialog = false;
          }
        }
      },
      ...
      change(){
        ...
        if (checks.gta(data)){
          if (confirm(`Вы действительно хотите обновить ${this.name} ${id}?\n${JSON.stringify(data)}`)){
            ...
            this.$store.dispatch('updateData', {id, data}); // Change in other components
          }
        }
      },
      deleteData(data){
        if (data.current === true)
        {
          ...
          this.$store.dispatch('deleteData', data); // Change in other components
        }
      }
    },
    mounted() {
      this.$store.dispatch('fetchDatas'); // Change in other components
    }
  }
</script>
